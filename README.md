...
| 5 | **Gerenciamento de Clientes e Usuários** | CRUD de clientes (PF/PJ) e dos usuários vinculados, incluindo ativação, reset de senha (via fluxo do auth-service) e invalidação de sessão (`logout` remoto / `tokenVersion`). **Detalhe do usuário** (`/usuarios/:id`): dados resumidos, atalhos e seção de **permissões diretas** (POST/DELETE `/users/{id}/permissions`) quando o operador possui leitura do catálogo de permissões e permissão de atribuição ao usuário. | `perm:Clients.*` · `perm:Users.*` <!-- TODO: confirmar --> |
